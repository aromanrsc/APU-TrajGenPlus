...
import utils.ldptrace.utils as utils
import numpy as np
from utils.ldptrace.grid import Grid, GridMap
...
def calculate_coverage_kendall_tau(orig_db: List[List[Grid]],
    ...
    for i in range(len(grid_map.map)):
        for j in range(len(grid_map.map[0])):
            ...
            for t in orig_db:
                actual_counts[index] += utils.ldptrace.trajectory.pass_through(t, g)
            for t in syn_db:
                syn_counts[index] += utils.ldptrace.trajectory.pass_through(t, g)
...
def calculate_length_error(orig_db: List[List[Tuple[float, float]]],
                           syn_db: List[List[Tuple[float, float]]],
                           bucket_num=20):
    orig_length = [get_travel_distance(t) for t in orig_db]
    syn_length = [get_travel_distance(t) for t in syn_db]
